    ...
    async def _async_update_data(self) -> ApSystemsSensorData:
        ...
        try:
            ...
            if (self.last_update_day != dt_util.now().day):
                self.last_update_day = dt_util.now().day
                if output_data.e1 < (self.last_dayp1 - 0.0002): self.last_dayp1 = output_data.e1 # if the day reset by chance happens exactly at the day reset, we need to update the last_dayp1 to prevent false reset value
                if output_data.e2 < (self.last_dayp2 - 0.0002): self.last_dayp2 = output_data.e2
                ...
                if output_data.e1 < (self.last_dayp1 - 0.0002):  # we assume a day production is bigger than 0.0002 kWh, if not detection will fail. But this is no further issue
                ...
                if output_data.e2 < (self.last_dayp2 - 0.0002):
                    # This means that the day production has been reset, so we update the base day produced values
                    self.base_day_p2 += self.last_dayp2
